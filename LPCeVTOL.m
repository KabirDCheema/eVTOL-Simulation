function [problem,guess] = LPCeVTOL
%
...
InternalDynamics=@Copy_of_copy_of_robot;
SimDynamics=@Copy_of_robot_sim;
...
% Settings file
problem.settings=@settings_LPCeVTOL;
...
problem.states.x0=[xi zi 0 0 0 0 0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[xi zi 0 0 0 0 0]; 
problem.states.x0u=[xi zi 0 0 0 0 0]; 

% State bounds. xl=< x <=xu
problem.states.xl=[-inf inf -inf -inf -.2 -.2 -.15];
problem.states.xu=[inf inf inf inf .2 .2 .15];

% State error bounds
problem.states.xErrorTol_local=[1e-3 1e-3 1e-3 1e-3 1e-3 1e-3 1e-3];
problem.states.xErrorTol_integral=[1e-3 1e-3 1e-3 1e-3 1e-3 1e-3 1e-3];

% State constraint error bounds
problem.states.xConstraintTol=[1e-3 1e-3 1e-3 1e-3 1e-3 1e-3 1e-3];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[xf zf 0 0 0 0 0]; 
problem.states.xfu=[xf zf 0 0 0 0 0];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[xi xf];
guess.states(:,2)=[xi zf];
guess.states(:,3)=[0 0];
guess.states(:,4)=[0 0];
guess.states(:,5)=[0 0];
guess.states(:,6)=[0 0];
...
% Input bounds
problem.inputs.ul=[-1000 -1000];
problem.inputs.uu=[1000 1000];

% Bounds on the first control action
problem.inputs.u0l=[0 0];
problem.inputs.u0u=[10 10];

% Input constraint error bounds
problem.inputs.uConstraintTol=[1e-3 1e-3];
